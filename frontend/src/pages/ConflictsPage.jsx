...
export default function ConflictsPage() {
  ...
  const onApplyFix = async () => {
    if (!conflict?.generation_id) {
      setStatus("Load a valid generation first.");
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {conflict && (
        <Card>
          <CardHeader>
            <CardTitle>{conflict.has_conflict ? "Conflict Found" : "No Conflict"}</CardTitle>
            <CardDescription>{conflict.message}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">Generation: {conflict.generation_id || "None"}</p>
